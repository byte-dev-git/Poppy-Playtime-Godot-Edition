...
func _update_lean(delta: float, sk: Skeleton3D):
	...
	if lean_inertia_enabled:
		# Пружина-демпфер: сила = stiffness*(target - current) - damping*velocity
	...
	if lean_pitch_enabled:
		var pitch = cam.global_rotation.y
...
func _update_lookat(delta: float, sk: Skeleton3D):
	...
	var bone_global_origin = sk.global_transform * bone_pose.origin

	var destination: Vector3
	if lookat_target:
		destination = lookat_target.global_transform * lookat_target_offset
	else:
		destination = bone_global_origin + (sk.global_transform.basis * bone_pose.basis).z * 2.0
	...
	var local_target = sk_inv * _lookat_lerped_target

	if local_target.distance_to(bone_pose.origin) < 0.001: return

	var dir = (local_target - bone_pose.origin).normalized()
